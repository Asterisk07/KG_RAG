...
x12 = os.environ["GOOGLE_API_KEY"]
print(f"api key is {x12[:5]} (LINE BEING PRINTED FROM utility.py)")
...
def fetch_GPT_response(instruction, system_prompt, chat_model_id, chat_deployment_id, temperature=0):
    print("fetching gpt resonse")
    response = openai.ChatCompletion.create(
        temperature=temperature,
        deployment_id=chat_deployment_id,  # tk this was disabled
        model=chat_model_id,
        messages=[
...
def disease_entity_extractor_v2(text, model_id=None):
    chat_model_id, chat_deployment_id = get_gpt35()
    prompt_updated = system_prompts["DISEASE_ENTITY_EXTRACTION"] + \
        "\n" + "Sentence : " + text
    resp = get_GPT_response(
        prompt_updated, system_prompts["DISEASE_ENTITY_EXTRACTION"], chat_model_id, chat_deployment_id, temperature=0)
    try:
        entity_dict = json.loads(resp)
        return entity_dict["Diseases"]
    except:
